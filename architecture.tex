...
\begin{document}
    ...
    \subsection{Purpose}
   This document provides a comprehensive architectural overview of the system, using a number of different architectural views to depict different aspects of the system. We focused on using the Model-View-Controller architecture system. Our document intends to capture and convey the significant architectural decisions which have been made on the system. 
	\subsection{Scope}
    This software architectural design document provides an architectural overview of the NavUP system and the Data, Events, Points of Interest and User Management subsystems. NavUP is being developed at the University of Pretoria to aid in navigation around the main campus in Hatfield.
...
Application icons visually express the application or brands product and services. For example, the NavUP logo or icon. It would be simple, bold and friendly to communicate the core ide and intent of the product/application. To help user navigate their way around campus, in this case. The application icon is the brand identity.
	...
	The images bellow are mearly mock designs of the NavUP User Interface
	
	\includegraphics[width=0.5\linewidth]{Images/screenDesigns.jpg}\\[1cm]
	\includegraphics[width=0.5\linewidth]{Images/homeScreen.png}\\[1cm]
...
\end{enumerate}
	...
	We will be using passportJS as authentication middleware for the user passowrd verification. This will then be used to interface with the datbase manager unit.
	It is one of nodeJS's extensions. 
	We will also be using StormPath as a usermanagement API. 
	\subsection{Events}
...
The Application class can be seen as an abstraction of the Access Module and contains operations such as "syncCalendar()" and "postToSocialMedia()" which are both important in the functionality of the Events Module. These functions, as their names suggest, synchronize events with a calendar (either the users calendar or a calendar that ships with the application) and posts these new events to social media such as Facebook and Instagram in order to create awareness of the event.
	...
	\begin{figure}[!h]
        \centering
        \includegraphics[width=\textwidth]{Diagrams/PointsOfInterest/POIUse}
